...
#define MAX_FLASH_LENGTH ((1024U * 512U) - BOOTLOADER_SIZE)
...
int main(void)
{
  ...
  while (state != BL_State_Done)
  {
    ...
    /*Now that we are synced, all further comms should be packets*/
    ...
    case BL_State_FWLengthRes:
    {
      if (comms_packets_available())
      {
        ...
        fw_length = ((temp_packet.data[1]) |
                     (temp_packet.data[2] << 8) |
                     (temp_packet.data[3] << 16) |
                     (temp_packet.data[4] << 24));
        if (is_fw_length_packet(&temp_packet) && (fw_length <= MAX_FLASH_LENGTH))
        {
          state = BL_State_EraseApplicaion;
        }
        else
        {
          bootloading_fail();
