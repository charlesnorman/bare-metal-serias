...
const PACKET_LENGTH =
  PACKET_LENGTH_BYTES + PACKET_DATA_BYTES + PACKET_CRC_BYTES;
...
uart.on("data", (data) => {
  ...
  while (rxBuffer.length >= PACKET_LENGTH) {
    ...
    const packet = new Packet(
      raw[0],
      raw.slice(1, 1 + PACKET_DATA_BYTES),
      raw[PACKET_CRC_INDEX]
    );
    // console.log(packet);
    ...
    .then((packet) => {
      if (packet.length !== 1 || packet.data[0] !== byte) {
        const formattedPacket = [...packet.toBuffer()]
          ...
          .join(" ");
        throw new Error(
          `Unexpected packet received. Expected single byte 0x${byte.toString(
            16
          )}), got packet ${formattedPacket}`
        );
      }
    })
    .catch((e: Error) => {
...
const main = async () => {
  ...
  const fwUpdatePacket = Packet.createSingleBytePacket(
  ...
  /********************* Send DeviceID *********************/
  const deviceIDPacket = new Packet(
    2,
    Buffer.from([BL_PACKET_DEVICE_ID_RES_DATA0, DEVICE_ID])
  );
  ...
  while (bytesWritten < fwLength) {
    ...
    const dataBytes = fwImage.slice(
      bytesWritten,
      bytesWritten + PACKET_DATA_BYTES
    );
